...
def calculate_scores(filename, output_filename):
    cmd_vmaf = (
        f"ffmpeg -i {filename} -i {output_filename} -filter_complex libvmaf -f null -"
    )
...
if __name__ == "__main__":
    filename = "videoplayback.mp4"  # This should be fetched dynamically
    download_file(filename)
    output_filename = convert_to_av1(filename)
    calculate_scores(filename, output_filename)
